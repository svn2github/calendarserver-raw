...
class StatsAction(object):
    ...
    def run(self):
        assert self.root.exists(), "Address book server document root directory does not exist: %s" % (self.root.path,)
        assert os.access(self.root.path, os.R_OK), "Cannot read address book server document root directory: %s" % (self.root.path,)
...
class StatsWatchAction(object):
    ...
    bin/cardadmin --config conf/carddavd-dev.plist statswatch
    bin/cardadmin --config conf/carddavd-dev.plist statswatch --refresh 10
    """
        ...
        if self.refresh is None:
            ...
            while 1:
                print "Total hits: %10d\tLast %7s: %8d\tRate: %8.2f" % (
                    total, "%dm%02ds" % (since / 60, since % 60),
