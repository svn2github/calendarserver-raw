...
defaultConfigFile = "/etc/carddavd/carddavd.plist"
...
defaultConfig = {
    ...
    "HTTPPort": 8800,        # HTTP port (0 to disable HTTP)
    ...
    #
    "EnableCalDAV"  : False,  # Enable CalDAV service
    "EnableCardDAV" : True,  # Enable CardDAV service

    #
    # Data store
    #
    "DataRoot"                : "/Library/AddressBookServer/Data",
    "DocumentRoot"            : "/Library/AddressBookServer/Documents",
    ...
    "DirectoryService": {
        ...
        "params": serviceDefaultParams["twistedcaldav.directory.appleopendirectory.OpenDirectoryService"],
    },
    ...
    "SudoersFile": "",
    "EnableProxyPrincipals": False,               # Create "proxy access" principals

    #
    # Permissions
    #
    "EnableAnonymousReadRoot": False,    # Allow unauthenticated read access to /
    "EnableAnonymousReadNav": False,    # Allow unauthenticated read access to hierachcy
    "EnablePrincipalListings": False,    # Allow listing of principal collections
    ...
    # Logging
    #
    "AccessLogFile"  : "/var/log/carddavd/access.log",  # Apache-style access log
    "ErrorLogFile"   : "/var/log/carddavd/error.log",   # Server activity log
    "ServerStatsFile": "/var/run/carddavd/stats.plist",
    "PIDFile"        : "/var/run/carddavd.pid",
    "GlobalStatsSocket"           : "/var/run/carddavd-stats.sock",
    ...
    "AccountingLogRoot": "/var/log/carddavd/accounting",
    ...
    "Scheduling": {
        ...
        "CalDAV": {
            ...
            "OldDraftCompatibility"      : False,  # Whether to maintain compatibility with non-implicit mode
            "ScheduleTagCompatibility"   : False,  # Whether to support older clients that do not use Schedule-Tag feature
            "EnablePrivateComments"      : False,  # Private comments from attendees to organizer
        },
    ...
    # Twisted
    "Twisted": {
        "twistd": "/usr/share/carddavd/bin/twistd",
        "reactor": "select",
    },

    # Python Director
    "PythonDirector": {
        "pydir": "/usr/share/carddavd/bin/pydir.py",
        "ConfigFile": "/etc/carddavd/pydir.xml",
        "ControlSocket": "/var/run/carddavd-pydir.sock",
    },
    ...
    "ControlSocket": "/var/run/carddavd.sock",
...
class Config (object):
    ...
    def __init__(self, defaults):
        ...
        self._hooks = [
            ...
            self.updateDirectoryAddressBookBackingDirectoryService,
            self.updateACLs,
    ...
    @staticmethod
    def updateDirectoryAddressBookBackingDirectoryService(self, items):
        #
        # Special handling for directory address book configs
        #
        dsType = items.get("DirectoryAddressBook", {}).get("type", None)
        if dsType is None:
            dsType = self._data["DirectoryAddressBook"]["type"]
        else:
            if dsType == self._data["DirectoryAddressBook"]["type"]:
                oldParams = self._data["DirectoryAddressBook"]["params"]
                newParams = items["DirectoryAddressBook"].get("params", {})
                _mergeData(oldParams, newParams)
            else:
                if dsType in directoryAddressBookBackingServiceDefaultParams:
                    self._data["DirectoryAddressBook"]["params"] = copy.deepcopy(directoryAddressBookBackingServiceDefaultParams[dsType])
                else:
                    self._data["DirectoryAddressBook"]["params"] = {}
        ...
        for param in tuple(self._data["DirectoryAddressBook"]["params"]):
            if param not in directoryAddressBookBackingServiceDefaultParams[self._data["DirectoryAddressBook"]["type"]]:
                del self._data["DirectoryAddressBook"]["params"][param]
    ...
    @staticmethod
    def updateAddressBook(self, items):
        #
        # FIXME: Use the config object instead of doing this here
        #
        from twistedcaldav.resource import CalendarPrincipalResource
        CalendarPrincipalResource.enableAddressBooks(self.EnableCardDAV)
    ...
    def loadConfig(self, configFile):
        ...
        if configFile and os.path.exists(configFile):
            ...
            log.warn("Configuration file does not exist or is inaccessible: %s" % (configFile,))
