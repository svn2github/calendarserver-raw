...
class RotatingFileAccessLoggingObserver(CommonAccessLoggingObserverExtensions):
    ...
    def rotateGlobalHitHistoryStats(self):
        ...
        if config.GlobalStatsLoggingFrequency is not 0:
            self.reactor.callLater(config.GlobalStatsLoggingPeriod * 60 / config.GlobalStatsLoggingFrequency,
    ...
    def getGlobalHits(self):
        ...
        return stats % (self.globalHitCount, self.globalHitCount - self.globalHitHistory[0]['hits'],
                        self.globalHitHistory[0]['time'], config.GlobalStatsLoggingPeriod, config.GlobalStatsLoggingFrequency)
