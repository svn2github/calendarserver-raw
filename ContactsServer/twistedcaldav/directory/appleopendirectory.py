...
class OpenDirectoryService(CachingDirectoryService):
    ...
    def reloadCache(self, recordType, lookup=None, logIt=True):
        # make sure the record is fresh
        if lookup is not None:
            record = {
                "shortName" : self.recordWithShortName(recordType, lookup[1]),
                "guid"      : self.recordWithGUID(lookup[1]),
                "authID"    : self.recordWithAuthID(lookup[1]),
                "email"     : self.recordWithEmailAddress(lookup[1]),
            }.get(lookup[0])
            
            if record:
                import time
                slopTime = 5
                if (time.time() - record.cachedTime > slopTime):
    ...
    def isAvailable(self):
        ...
        self.log_warn("isAvailable returns TRUE" )
        return True
