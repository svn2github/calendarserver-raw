...
from twistedcaldav.static import CalendarHomeFile, AddressBookHomeFile
...
class RootResource (ReadOnlyResourceMixIn, DirectoryPrincipalPropertySearchMixIn, DAVFile):
    ...
    def __init__(self, path, *args, **kwargs):
        ...
        if config.Memcached["ClientEnabled"]:
            ...
            AddressBookHomeFile.cacheNotifierFactory = MemcacheChangeNotifier
        ...
        if not RootResource.serverUUID:
            ...
            try:
                ...
                RootResource.serverUUID = str(UUID(uuidFile.read()))
            except Exception, e:
                log.error("Failed to read server UUID from file \"%s\". ERROR: %s" % (serverUUIDFilePath, e,))
                RootResource.serverUUID = str(uuidFromName("CF09BA68-367A-11DE-A97A-0017F201C138", str(getnode()))).upper()
            finally:
                if uuidFile:
                    uuidFile.close()    
            log.info("Server UUID is %s" % RootResource.serverUUID)
    ...
    liveProperties = tuple(DAVFile.liveProperties) + (
        (dav_namespace, "resourceID"),
    )
    ...
    def readProperty(self, property, request):
        ...
        if namespace == dav_namespace:
            if name == "resourceID":
                returnValue(customxml.ResourceID(RootResource.serverUUID))
        
        result = (yield super(RootResource, self).readProperty(property, request))
        returnValue(result)
                ...
                try:
                    ...
                    if record is None:
                        raise HTTPError(StatusResponse(
                            ...
                            "The username (%s) corresponding to your sessionID was not found by address book server." % (username,)
                        ))
...
try:
    from addressbookserver.platform.darwin._sacl import CheckSACL
