...
class CardDAVStatisticsProtocol (Protocol):
    def connectionMade(self):
        stats = self.factory.logger.observer.getGlobalHits()
        self.transport.write("%s\r\n" % (stats,))
        self.transport.loseConnection()

class CardDAVStatisticsServer (Factory):
    protocol = CardDAVStatisticsProtocol
    def __init__(self, logObserver):
        self.logger = logObserver

class CardDAVService (MultiService):
    def __init__(self, logObserver):
...
class CardDAVOptions (Options, LoggingMixIn):
    optParameters = [[
    ...
    def __init__(self, *args, **kwargs):
        super(CardDAVOptions, self).__init__(*args, **kwargs)
    ...
    def postOptions(self):
        ...
        self.checkDirectory(
            ...
            # address book server requires write access <rdar://6627766>
            access=os.W_OK,
...
class CardDAVServiceMaker (LoggingMixIn):
        ...
        if not serviceMethod:
            ...
            def sighup_handler(num, frame):
                ...
                if config.ProcessType == "Combined":
                    service.processMonitor.signalAll(signal.SIGHUP, "carddav")
    ...
    def makeService_Slave(self, options):
        ...
        waitTime = 0
        maxWait = config.DirectoryService.params.get("maxAvailableWaitSeconds",-1)
        while not baseDirectory.isAvailable() and (maxWait < 0 or waitTime < maxWait):
            sleep(5)
            waitTime += 5
        ...
        if config.EnableCardDAV:
            ...
            if config.DirectoryAddressBook and config.EnableSearchAddressBook:
        ...
        service = CardDAVService(logObserver)
    ...
    def makeService_Combined(self, options):
        ...
        if config.MultiProcess.ProcessCount == 0:
            ...
            if 1 * cpuCount > processCount:
                processCount = 1 * cpuCount
        ...
        stats = CardDAVStatisticsServer(logger)
        statsService = UNIXServer(config.GlobalStatsSocket, stats)
        statsService.setServiceParent(s)
        
        #check for server UUID in file 
        serverUUIDFilePath = os.path.join( config.DocumentRoot, ".server-uuid" )
        uuidFile = None
        try:
            # try to make a UUID with file content or fail
            uuidFile = open(serverUUIDFilePath)
            UUID(uuidFile.read())
        except Exception, e:
            if e.errno != errno.ENOENT:
                log.werror("Failed to read server UUID from file \"%s\". ERROR: %s" % (serverUUIDFilePath, e,))
            if uuidFile:
                uuidFile.close()
                uuidFile = None
            
            # write out a new UUID file
            try:
                uuidFile = open(serverUUIDFilePath, "w")
                uuidStr = str(uuid4()).upper()
                uuidFile.write(uuidStr)
                self.log_info("Setting server UUID to %s in file \"%s\"." % (uuidStr, serverUUIDFilePath))
                if config.UserName and config.GroupName:
                    uid = getpwnam(config.UserName)[2]
                    gid = getgrnam(config.GroupName)[2]
                    os.chown(serverUUIDFilePath, uid, gid)

            except Exception, e:
                log.error("Failed to write server UUID to %s. ERROR: %s" % (serverUUIDFilePath, e,))
