...
def version():
    ...
    branches = (
        "tags/release/AddressBookServer-" + base_version,
        "branches/release/AddressBookServer-" + base_version + "-dev",
        "trunk",
    )
    ...
    for branch in branches:
        ...
        if svn_revision == "exported":
            if "RC_JASPER" in os.environ and os.environ["RC_JASPER"] == "YES":
                ...
                prefix = "AddressBookServer-"

                if project_name.startswith(prefix):
                    rc_version = project_name[len(prefix):]
                    if "." in rc_version:
                        comment = "Address Book Server v%s" % (rc_version,)
                    else:
                        comment = "Address Book Server [dev] r%s" % (rc_version,)
                    break
